(function () {
  ...
  function setTimeoutForRequest(requestId) {
    __requestIdVsTimeout[requestId] = setTimeout(
      function (requestId) {
  ...
  function processPostMessage(event) {
    if (!isValidMessage(event)) {
      console.error(
        "ameyo_aap_sdk : message received from a different origin than ameyo host application"
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "ameyo_aap_sdk : invalid json response returned from ameyo application " +
          e
      );
    }
  }
  ...
  function handleApplicationResponse(responseObject) {
    ...
    if (responseObject.responseType == "request") {
      ...
      if (!callbackObject) {
        console.log(
          "ameyo_aap_sdk : ameyo application response received after request timeout. service : " +
      ...
      var callback =
        __requestIdVsCallback[responseObject.requestId][responseObject.status];
  ...
  var Client = function (data) {
    ...
    this.request = {
      api: function (requestName, requestParams) {
        return new Promise(function (resolve, reject) {
          var requestId = getUniqueID();
          var request = {
            requestId: requestId,
